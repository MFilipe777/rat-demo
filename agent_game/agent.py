...
class Agent:
    ...
    def get_state(self, game):
        ...
        if x >= 0 and x <= game.grid.n_cols:
            wall_right = game.grid.grid[x + 1][y] == 3
            wall_left = game.grid.grid[x - 1][y] == 3
        else:
            wall_right, wall_left = True
        
        if y >= 0 and y <= game.grid.n_rows:
            wall_up = game.grid.grid[x][y-1] == 3
            wall_down = game.grid.grid[x][y+1] == 3
        else:
            wall_up, wall_down = True

        state  = [x, y, goal_left, goal_right, goal_up, goal_down, wall_left, wall_right, wall_up, wall_down]
        #print(state)
